...
export class ProcessVehicleInventoryService {
  ...
  async processFolder(folderPath: string) {
    ...
    for (const file of files) {
      ...
      const counters: Record<VehicleAction, number> = { added: 0, updated: 0, noChange: 0, skipped: 0, total: 0, deleted: 0 };
      ...
      try {
        ...
        for await (const record of parser) {
          counters.total++;
          try {
            const vin = normalizeString(record[INVENTORY_CONST.CSV_HEADERS.VIN]);
            const dealerId = normalizeString(record[INVENTORY_CONST.CSV_HEADERS.DEALER_ID]);
            if (!vin || !dealerId) {
              counters.skipped++;
              continue;
            }
            ...
            const { action } = await this.UpsertVehicleService.upsertVehicle(record, vin, dealerId);
            this.handleAction(action, counters);

            console.log(` VIN ${vin} | ${INVENTORY_CONST.ACTION_LOGS[action]}`);
          } catch (err: any) {
            console.error(`Row ${counters.total} VIN ${record['VIN'] || 'N/A'} failed: ${err.message}`);
          }
        }
        ...
        if (currentRooftopId && processedVins.size) {
          ...
          counters.deleted = Array.isArray(deletedRows) ? deletedRows.length : deletedRows;
        }

        await this.importFileJobService.completeJob(job.ifj_id, counters.total, counters.skipped, counters.added, counters.updated, counters.noChange, counters.deleted);
      } catch (err: any) {
  ...
  private handleAction(action: VehicleAction, counters: Record<VehicleAction, number>) {
    counters[action]++;
  }
}
