...
import { VEHICLES_COLUMN_MAPPING, VEHICLE_HISTORY_COLUMN_MAPPING_MONGO, } from '../shared/db.mapping';
import { mapCsvRecordToDbObject } from 'src/utils/csv-to-db-mapper';
import { INVENTORY_CONST, TABLE_NAMES, VehicleAction } from '../shared/vehicle.constants';
...
import { normalizeBoolean, normalizeNullableNumber, normalizeNumber, normalizeString, normalizeStringArray } from 'src/utils/safe-trim-value';
...
export class VehicleUpsertService {
  ...
  async upsertVehicle(record: any, vin: string, dealerId: string): Promise<{ action: VehicleAction }> {
    ...
    const imagesCsv = normalizeStringArray(record[INVENTORY_CONST.CSV_HEADERS.IMAGE_LIST]);

    vehCsvData.veh_listing_type = this.normalizeListingType(vehCsvData.veh_listing_type);
    ...
    const veh_year_id = await this.vehicleService.VehicleLookupId(TABLE_NAMES.VEHICLE_YEAR, 'year', normalizeNumber(record[INVENTORY_CONST.CSV_HEADERS.YEAR]));
    const veh_body_type_id = await this.vehicleService.VehicleLookupId(TABLE_NAMES.VEHICLE_BODY_TYPE, 'body_type', normalizeString(record[INVENTORY_CONST.CSV_HEADERS.BODY_TYPE]));
    const veh_ext_color_id = await this.vehicleService.VehicleLookupId(TABLE_NAMES.VEHICLE_COLOR, 'color', normalizeString(record[INVENTORY_CONST.CSV_HEADERS.EXT_COLOR]));
    const veh_int_color_id = await this.vehicleService.VehicleLookupId(TABLE_NAMES.VEHICLE_COLOR, 'color', normalizeString(record[INVENTORY_CONST.CSV_HEADERS.INT_COLOR]));
    ...
    const veh_Exist = await this.db(TABLE_NAMES.VEHICLES).select('veh_id', 'veh_options_mng_id', 'veh_description_mng_id').where({ veh_vin: vin }).first();
    ...
    if (!veh_Exist) {
      ...
      await this.db(TABLE_NAMES.VEHICLES).where({ veh_id: vehicleId }).update({
    ...
    await this.db(TABLE_NAMES.VEHICLES).where({ veh_id: vehicleId }).update({
      ...
      veh_description_mng_id: mongoIds.descriptionId ?? veh_Exist.veh_description_mng_id,
    });
