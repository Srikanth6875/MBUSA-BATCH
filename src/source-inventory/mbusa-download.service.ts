...
import { INVENTORY_CONST } from 'src/shared/vehicle.constants';
import { DownloadResult, DownloadServerFile } from 'src/shared/type.config';
...
export class MbusaDownloadService {
    ...
    async downloadLatestFile(targetDir: string): Promise<DownloadResult> {
        ...
        const preText = preMatch[1].replace(/<a[^>]*>/g, '').replace(/<\/a>/g, '').replace(/&nbsp;/g, ' ');
        // Split lines by <br>
        ...
        for (const line of lines) {
            ...
            const match = trimmed.match(/\d{1,2}\/\d{1,2}\/\d{4}\s+\d{1,2}:\d{2}\s+[AP]M\s+(\d+)\s+<A HREF="[^"]*">(MBUSA\d{14}\.csv)<\/A>/i);
            if (match) serverFiles.push({ name: `/MBUSA/${match[2]}`, size: Number(match[1]), });
        }
        ...
        const lastJob = await this.db('import_jobs')
            .where({ ij_source: 'MBUSA', ij_file_name: serverFileName })
            .orderBy('ij_id', 'desc').first();

        if (lastJob && Number(lastJob.ij_file_size) === serverFileSize) {
        ...
        for (const f of fs.readdirSync(targetDir)) { // Delete old local CSVs
        ...
        await curlDownload({ url: fileUrl, output: filePath, timeoutMs: 600_000, retries: 3, });
